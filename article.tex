...
\begin{equation}
S(t) = \prod_{i=1}^{n} \left(1 - \frac{d_i}{r_i}\right),
\end{equation}
...
We are not interested in the individual \(p_i^\text{survived}\) values, but rather the overall survival probability \(S(t)\).  We therefore minimize the NLL over the \(p_i^\text{survived}\) with the constraint
\begin{equation}
\prod_{i=1}^{n} p_i^\text{survived} = S(t). \label{eq:km-probability-constraint}
\end{equation}

\subsection{Patient-wise uncertainty}

We now consider that some patients' parameters may be subject to uncertainty.  Some patients who were nominally included in the Kaplan--Meier curve may actually be excluded or vice versa.
...
In this release of \pkg{KoMbine}, we support the Poisson uncertainty, optionally normalized by a constant, or the ratio of two Poisson uncertainties.  This is ideal for uncertainties such as the density of a cell type.  The Kaplan--Meier curve is constructed from patients whose parameters fall within a certain range.

It is calculated as the difference between the minimum NLL for the patient to be included and the minimum NLL for the patient to be excluded (or, for the functions we consider here, the signed difference between the NLL at the patient's parameter and the NLL at the boundary of the range).

For the purpose of the math here, all that is important is that the penalty is a fixed number per patient.  We compute the NLL as
\begin{equation}
-\log \mathcal{L}_{\text{patient}}(a_j) = -\sum_{j=1}^{m} a_j\log \mathcal{L}_j^{\text{patient}},
\end{equation}
where \(\mathcal{L}_j^\text{patient}\) is the penalty for patient \(j\) to be included, and \(a_j=1\) if the patient is included and \(0\) if the patient is excluded.
...
\end{equation}
Now that we have added the patient-wise uncertainty, \(r_i\) and \(d_i\) are no longer fixed, but depend on which patients are included or excluded:
\begin{align}
r_i &= \sum_{t_j \geq t_i} a_j \\
d_i &= \sum_{\substack{t_j = t_i \\ \neg c_j}} a_j
\end{align}
where \(c_j=1\) if the patient is censored and \(0\) otherwise.  The binomial penalty therefore also depends on the patient inclusion indicators \(a_j\).

We minimize the overall NLL \cref{eq:total-nll} over the \(p_i^\text{survived}\) and \(a_j\), again with the constraint \cref{eq:km-probability-constraint}.
...
To examine the binomial uncertainty, we can fix \(a_j\) to the nominal values of the uncertainties.  These results are approximately equivalent to the Greenwood confidence intervals used in other packages (our code is somewhat more precise, but also slower), as will be illustrated in \cref{sec:compare-to-greenwood}.

Isolating the patient-wise uncertainty is more nontrivial.  Without the binomial penalty term, minimizing \(-\log \mathcal{L}_{\text{patient}}\) over \(a_j\) will simply put each patient's parameter at its nominal value.

\section{Implementation}

\pkg{KoMbine} is implemented in \proglang{Python} and uses \pkg{Gurobi} to accomplish the optimization for each value of \(S(t)\).  \pkg{scipy} is then used to minimize \(-\log \mathcal{L}_{\text{total}}\) over \(S(t)\) to find the maximum likelihood estimate and to compute the confidence intervals, bounded by the points where the NLL equals a threshold value (1.0 for 68\% CL and 3.84 for 95\% CL).
