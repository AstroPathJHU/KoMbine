...
A primary limitation of our current framework is that it is limited to uncorrelated systematic errors.  Extending the methodology to uncertainties that are correlated between patients, such as batch effects, would involve modifying our minimization problem structure to float the uncertainty parameters and computing \(a_j\) from them, rather than precomputing penalties and floating \(a_j\) directly.  Although straightforward to describe, this conceptual switch is nontrivial to implement within the current optimization framework described in Sec.~\ref{sec:implementation-details} and will probably require a rethinking of the optimization strategy.
