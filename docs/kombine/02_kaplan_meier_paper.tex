...
\pkg{KoMbine} is available from PyPI via \code{pip install kobmine} and from GitHub at \url{https://github.com/AstroPathJHU/KoMbine}.
...
\pkg{KoMbine} automatically collapses such time points before performing the NLL minimization. When applying binomial-only errors, the second line only contributes a constant offset to the NLL\@. Although it does affect the NLL when patient-wise errors are included, it can be precomputed for the various possible values of \(d_i\) and implemented in \pkg{Gurobi} using indicator variables, thereby reducing the number of continuous variables. This can significantly speed up the NLL minimization, especially when not too many patients are censored.
...
\bibliography{02_kaplan_meier_paper}
