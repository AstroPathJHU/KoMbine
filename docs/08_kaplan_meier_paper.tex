...
\label{sec:collapsing-consecutive-deaths}
...
\begin{align}
\begin{aligned}
-\log \mathcal{L}_{\text{binomial},1,2} =&-\left[\log\binom{r_1}{d_1} + d_1 \log \left(1-p_1^\text{survived}\right) + (r_1-d_1) \log p_1^\text{survived}\right] \\
&-\left[\log\binom{r_{1}-d_{1}}{d_{2}} + d_{2} \log \left(1-p_{2}^\text{survived}\right) + (r_{1}-d_{1}-d_{2}) \log p_{2}^\text{survived}\right].
\end{aligned}
\label{eq:two-timepoints-nll}
\end{align}

The key observation is that \(p_1^\text{survived}\) and \(p_2^\text{survived}\) only enter the NLL in this term and in the constraint \cref{eq:km-survival}, which only depends on their product \(p_{1,2}^\text{survived}=p_1^\text{survived} p_2^\text{survived}\)\@. We can therefore minimize \cref{eq:two-timepoints-nll} over \(p_1^\text{survived}\) and \(p_2^\text{survived}\) with the constraint \(p_{1,2}^\text{survived}=p_1^\text{survived} p_2^\text{survived}\)\@.  The result is
\begin{align}
p_1^\text{survived} &= \frac{d_1 p_{1,2}^\text{survived}+d_2}{d_1+d_2}, \\
p_2^\text{survived} &= \frac{(d_1+d_2) p_{1,2}^\text{survived}}{d_1 p_{1,2}^\text{survived} + d_2}.
\end{align}

We plug into \cref{eq:two-timepoints-nll} to obtain:
\begin{align}
\begin{aligned}
-\log \mathcal{L}_{\text{binomial},1,2} =&-\left[\log\binom{r_1}{d_1+d_2} + (d_1+d_2) \log \left(1-p_{1,2}^\text{survived}\right) + (r_1-d_1-d_2) \log p_{1,2}^\text{survived}\right] \\
...
The first line of \cref{eq:two-timepoints-collapsed-nll} is identical to the NLL contribution from a single time point with \(d_1+d_2\) deaths and risk set \(r_1\)\@. The second line does not depend on \(p_{1,2}^\text{survived}\). When applying binomial-only errors, the second line only contributes a constant offset to the NLL\@. Although it does affect the NLL when patient-wise errors are included, it can be precomputed for the various possible values of \(d_1\) and \(d_2\) and implemented in \pkg{Gurobi} using indicator variables, thereby reducing the number of continuous variables.
...
\begin{align}
\begin{aligned}
-\log \mathcal{L}_{\text{binomial},1 \cdots n} =&-\left[\log\binom{r_1}{d_{1 \cdots n}} + d_{1 \cdots n} \log \left(1-p_{1 \cdots n}^\text{survived}\right) + (r_1-d_{1 \cdots n}) \log p_{1 \cdots n}^\text{survived}\right] \\
&-\left[\log{(d_{1 \cdots n}!)}-d_{1 \cdots n}\log d_{1 \cdots n} - \left(\sum_{i=1}^n \left[\log{(d_i!)} - d_i \log d_i\right]\right)\right],
\end{aligned}
\end{align}
with \(d_{1 \cdots n} = d_1 + d_2 + \cdots + d_n\) and \(p_{1 \cdots n}^\text{survived} = p_1^\text{survived} p_2^\text{survived} \cdots p_n^\text{survived}\)\@.
