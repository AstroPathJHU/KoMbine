...
&\approx-\sum_i \left[d_{i1} \log{H} - \sum_{m=0}^{d_{i0}+d_{i1}-1} \log\left(r_{i0} + H r_{i1} - m\right)\right] \label{eq:hypergeometric-nll-breslow}\\
&\approx-\sum_i \left[d_{i1} \log{H} - \log\left(r_{i0}+H r_{i1}\right)\right].
\label{eq:hypergeometric-nll-noties}
\end{align}
...
\Cref{eq:hypergeometric-nll-breslow} applies the Breslow approximation~\citep{breslow1974covariance}.  Both reduce to \cref{eq:hypergeometric-nll-noties} when no more than one patient dies at each time point.  Since our biomedical analyses typically do not involve many patients, ties are rare, and we therefore use \cref{eq:hypergeometric-nll-noties} in \pkg{KoMbine}.  We explore the effect of this approximation in \cref{sec:compare-to-conventional-p-value}.
