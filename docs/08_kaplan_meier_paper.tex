...
\label{sec:binomial-uncertainty}
...
\label{sec:patient-wise-uncertainty}
...
\label{sec:combining-uncertainties}
...
\label{sec:isolating-uncertainties}
...
The results in this paper were obtained using \proglang{Python}~3.11.13 with the \pkg{numpy}~2.3.2, \pkg{scipy}~1.16.1, and \pkg{gurobipy}~12.0.3 packages, as well as \pkg{Gurobi}~10.0.3.  All \proglang{Python} packages can be installed using \pkg{pip} or \pkg{conda}.

\pkg{Gurobi} can be downloaded from \url{https://www.gurobi.com/}.  Although it requires a license, free academic licenses are available.
