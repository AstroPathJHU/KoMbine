...
In the likelihood approach, we instead compute the full NLL, similar to \cref{eq:total-nll}.  We have two separate patient-wise NLL penalties, one for each curve.  The probability distribution representing the relationship between the curves is modeled with the Cox partial likelihood distribution (using the Breslow approximation) with \(\beta=\log{H}\), which reduces to the hypergeometric distribution under the null hypothesis when \(H=1\)~\citep{cox1972regression,breslow1974covariance}:
\begin{equation}
-\log \mathcal{L}_{\text{hypergeometric}}(a_{jk}, H)=-\sum_i \left[d_{i1} \log{H} - \sum_{m=0}^{d_{i0}+d_{i1}-1} \log\left(r_{i0} + H r_{i1} - m\right)\right]
\label{eq:hypergeometric-nll}
\end{equation}
