...
The binomial term in the NLL [\cref{eq:binomial-nll}] requires a sum over all time points where at least one patient died, which can be slow when there are many such time points.  For that reason, \pkg{KoMbine} can collapse consecutive deaths into a single time point, as long as no patients were censored in between.  For example, if a patient dies at time \(t_i\) from a risk set of \(r_i\), and then another two patients die at time \(t_{i+1}\) from a risk set of \(r_{i+1}=r_i-1\), the two death times can be combined into a single death of three patients at time \(t_{i+1}\) from a risk set of \(r_i\) for the purpose of calculating the likelihood at \(t_{i+1}\).  (When calculating the likelihood at \(t_i\), we do not include the deaths at \(t_{i+1}\).)

This speeds up the calculation without significantly affecting the likelihood and is enabled by default.  The size of the effect is explored in \cref{sec:compare-effect-of-collapsing}.
...
\subsection{Effect of collapsing consecutive deaths}\label{sec:compare-effect-of-collapsing}
