...
\Shorttitle{KoMbine: Propagating Errors to \KM{} Curves}
...
The \pkg{KoMbine} package provides the first method to propagate these types of errors through the \KM{} curve estimation process.  The techniques are based on the \pkg{Combine} package \citep{CAT-23-001}, developed by the CMS Collaboration in particle physics\@.  \pkg{Combine}, built on the \pkg{RooFit} framework \citep{RooFit}, is used for high-precision measurements of particle properties, which must account for tens or even hundreds of systematic uncertainties, and was used for such high profile measurements as the Higgs boson discovery \citep{HIG-12-028} and subsequent measurements of its properties.

\pkg{KoMbine} provides a similar interface to propagate uncertainties to \KM{} curves.  It handles the statistical uncertainty on the number of patients in a more precise way than the Greenwood confidence intervals and incorporates patient-wise uncertainties into the calculation.  The statistical and patient-wise uncertainties can also be plotted separately, allowing the user to see which uncertainties dominate.
...
We will first consider the binomial uncertainty and assume that the patients included in the \KM{} curve are fixed.

The binomial contribution to the \KM{} curve is computed from the number of patients at risk and the number of patients who died at each time point.  The nominal survival probability at time point \(t_n\) is given by the \KM{} formula
\begin{equation}
\hat{S}_n = \prod_{i=1}^{n} \left(1 - \frac{d_i}{r_i}\right), \label{eq:km-probability}
\end{equation}
...
We are not interested in the individual \(p_i^\text{survived}\) values, but rather the overall survival probability \(S_n\).  We therefore minimize the NLL over the \(p_i^\text{survived}\) with the constraint
\begin{equation}
\prod_{i=1}^{n} p_i^\text{survived} = \hat{S}_n \label{eq:km-probability-constraint}
\end{equation}
to find the likelihood \(\mathcal{L}_{\text{binomial}}(S_n)\) as a function of \(S_n\).

\subsection{Patient-wise uncertainty}

We now consider that some patients' parameters may be subject to uncertainty.  Some patients who were nominally included in the \KM{} curve may actually be excluded or vice versa.  In other words, \(d_i\) and \(r_i\) are no longer fixed numbers, but depend on which patients are included.  If we denote the inclusion of patient \(j\) by \(a_j\), then
\begin{align}
...
We use \code{scipy.optimize.differential_evolution} \citep{differential-evolution,scipy} to minimize \(-\log \mathcal{L}_{\text{total}}\) over \(S\) to find the maximum likelihood estimate\@.  \code{scipy.optimize.brentq} \citep{brentq,scipy} is then used to compute the confidence intervals, bounded by the points where the NLL equals a threshold value (1.0 for 68\% CL and 3.84 for 95\% CL).
...
for example, \code{scipy.stats.logpmf(4, 7, 4/7)=-1.225}, but \code{scipy.stats.logpmf(5, 8, 5/8)=-1.267}. %chktex 36
...
\end{equation}
where \(\hat{S}\) is the nominal value of the survival function, given by \cref{eq:km-probability}.  The exponential Greenwood interval is given by
\begin{equation}
\log{\left(-\log S(t)\right)} \in \log{\left(-\log \hat{S}(t)\right)} \pm z_{\alpha/2} \sqrt{\frac{1}{\log \hat{S}(t)^2} \sum_{i=1}^{n} \frac{d_i}{r_i(r_i-d_i)}}.
\end{equation}
...
\section[Introduction: Count data regression in R]{Introduction: Count data regression in \proglang{R}}\label{sec:example-intro}
...
out relative (dis)advantages. All software mentioned should be properly  %chktex 36
...
arguments, etc.). If there is such markup in (sub)section titles (as above), a  %chktex 36
...
econometrics, including  hurdle and zero-inflated models, is provided in~\cite{Cameron+Trivedi:2013}.
...
package%
%\citep{Jackman:2015}
. Of course, much more software could be discussed
...
in the {\LaTeX} code by \texttt{\%} lines) so that ``normal'' spacing is used and
...
\begin{Code}
glm(formula, data, subset, na.action, weights, offset, %chktex 36
  family = gaussian, start = NULL, control = glm.control(...), %chktex 11 chktex 36
  model = TRUE, y = TRUE, x = FALSE, ...) %chktex 11 chktex 26
\end{Code}
where \code{formula} plus \code{data} is the now standard way of specifying
regression relationships in \proglang{R}/\proglang{S} introduced in~\cite{Chambers+Hastie:1992}. The remaining arguments in the first line
...
\hline  %chktex 44
Type           & Distribution & Method   & Description \\ \hline  %chktex 44
...
\begin{CodeChunk}
\begin{CodeInput}
R> data("quine", package = "MASS") %chktex 36 chktex 18
\end{CodeInput}
\end{CodeChunk}
%
...
produced by \fct{Sweave} or \pkg{knitr} when using the \code{render_sweave()} %chktex 36
hook. Please make sure that all code is properly spaced, e.g., using
\code{y = a + b * x} and \emph{not} \code{y=a+b*x}. Moreover, code input should
use ``the usual'' command prompt in the respective software system. For
\proglang{R} code, the prompt \code{"R> "} should be used with \code{"+  "} as %chktex 18
the continuation prompt. Generally, comments within the code chunks should be
avoided---and made in the regular {\LaTeX} text instead. Finally, empty lines
before and after code input/output should be avoided (see above). %chktex 18
\end{leftbar}
...
\begin{CodeChunk}
\begin{CodeInput}
R> m_pois <- glm(Days ~ (Eth + Sex + Age + Lrn)^2, data = quine, %chktex 39 chktex 36
+    family = poisson)
\end{CodeInput}
\end{CodeChunk}
%
...
\begin{CodeChunk}
\begin{CodeInput}
R> library("MASS") %chktex 36 chktex 18
R> m_nbin <- glm.nb(Days ~ (Eth + Sex + Age + Lrn)^2, data = quine) %chktex 39 chktex 36
\end{CodeInput}
\end{CodeChunk}
%
...
\begin{CodeChunk}
\begin{CodeInput}
R> summary(m_nbin) %chktex 36
\end{CodeInput}
\begin{CodeOutput}
Call:
glm.nb(formula = Days ~ (Eth + Sex + Age + Lrn)^2, data = quine,  %chktex 39 chktex 36
    init.theta = 1.60364105, link = log)  %chktex 39
...
Coefficients: (1 not defined because of singularities)
            Estimate Std. Error z value Pr(>|z|)  %chktex 36   
...
---
Signif.\ codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1 %chktex 38

(Dispersion parameter for Negative Binomial(1.6036) family taken to be 1) %chktex 36

    Null deviance: 235.23  on 145  degrees of freedom
Residual deviance: 167.53  on 128  degrees of freedom
AIC: 1100.5 %chktex 13
...
\section[Using BibTeX]{Using Bib{\TeX}}\label{app:bibtex}
...
\begin{itemize}
  ...
  \item Software should be properly cited as well. For \proglang{R} packages
    \code{citation("pkgname")} typically provides a good starting point. %chktex 18 chktex 36
\end{itemize}
\end{leftbar}
