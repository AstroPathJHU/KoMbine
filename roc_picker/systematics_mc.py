...
def parse_datacard(file_path, id_start=0):
  ...
  for line in lines:
    ...
    if line.startswith("observable_type"):
      ...
      if data["observable_type"] != "fixed":
        raise ValueError("Unexpected 'observable' line for observable_type '{}'".format(data["observable_type"]))
      ...
      if data["observable_type"] != "poisson":
        raise ValueError("Unexpected 'count' line for observable_type '{}'".format(data["observable_type"]))
      ...
      if data["observable_type"] != "poisson_ratio":
        raise ValueError("Unexpected 'num' line for observable_type '{}'".format(data["observable_type"]))
      numerators = list(map(int, line.split()[1:]))
      continue
    elif line.startswith("denom"):
      if data["observable_type"] != "poisson_ratio":
        raise ValueError("Unexpected 'denom' line for observable_type '{}'".format(data["observable_type"]))
  ...
  if data["observable_type"] not in ["fixed", "poisson", "poisson_ratio"]:
    raise ValueError("Invalid observable_type: {}".format(data["observable_type"]))

  return data

def plot_from_datacard(file_path, saveas=None, *, id_start=0):
  """
  This function was generated using ChatGPT
  """
  data = parse_datacard(file_path, id_start=id_start)
  ...
  elif data["observable_type"] == "poisson":
    for p in data["patients"]:
  ...
  elif data["observable_type"] == "poisson_ratio":
    for p in data["patients"]:
  ...
  for systematic in data["systematics"]:
